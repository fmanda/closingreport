<template>
	<div>
		...
		<el-dialog
			title="Input Master Layanan"
			:visible.sync="dialogVisible"
			size="tiny"
			>
			<el-input placeholder="Kode Layanan" v-model="form.kode"></el-input>
			<el-input placeholder="Nama Layanan" v-model="form.nama"></el-input>
	...
	export default {
		data () {
			return {
				...
				items : [],
				fields : [
					{fieldname : 'kode', caption : 'Kode Layanan', width: 200},
					{fieldname : 'nama', caption : 'Nama Layanan', width: 200}
				],
				selectedfield : 'nama',
				form : {
					id: 0,
		...
		methods:{
			refreshData(reset){
				...
				var url = this.$rest_url + '/product/' + this.pagesize + '/' + this.currentpage + '/';
				var vm = this;
			...
			loadByID(id){
				...
				var vm = this;
				axios.get(this.$rest_url + '/product/' + id)
				.then(function(response) {
			...
			saveData(){
				var vm = this;
				axios.post(this.$rest_url + '/product', vm.form)
				.then(function(response) {
			...
			deleteData(item){
				var id = item.id;
				var vm = this;
				// axios.delete(this.$rest_url + '/product/' + id)
				axios.post(this.$rest_url + '/delete_product/' + id)
				.then(function(response) {
